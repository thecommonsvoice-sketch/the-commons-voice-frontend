...
export default function BusinessPage() {
  ...
  useEffect(() => {
    const fetchBusinessNews = async (): Promise<void> => {
      try {
        const res: Response = await fetch(`${API_BASE}/news/business`);
  ...
  return (
    <main className="relative p-6 max-w-7xl mx-auto">
      ...
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-yellow-200/20 rounded-full -z-10 blur-3xl" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((article, index) => (
            <div key={article.id} className="relative group">
              {/* Decorative “trending” badge on some cards */}
              {index % 4 === 0 && (
