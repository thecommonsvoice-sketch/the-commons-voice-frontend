...
import { Users, FileText, Shield, Search } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const toggleUserStatus = async (userId: string) => {
    try {
      const { data }: { data: { user: User } } = await api.patch(`/admin/users/${userId}/toggle`);
  ...
  const updateArticleStatus = async (articleId: string, status: Article["status"]) => {
    try {
      ...
      setArticles(prev => prev.map(article => (article.id === articleId ? { ...article, status } : article)));
    ...
    <RequireAuth allowedRoles={["ADMIN"]}>
      <div className="container mx-auto px-4 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card><CardHeader><CardTitle className="text-sm font-medium">Total Users</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{userCount}</div></CardContent></Card>
          <Card><CardHeader><CardTitle className="text-sm font-medium">Total Articles</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{articleCount}</div></CardContent></Card>
          <Card><CardHeader><CardTitle className="text-sm font-medium">Published Today</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{publishedToday}</div></CardContent></Card>
          <Card><CardHeader><CardTitle className="text-sm font-medium">Drafts</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{articles.filter(a => a.status === "DRAFT").length}</div></CardContent></Card>
        </div>
            ...
            {userLoading ? <div>Loading users...</div> : (
              <div className="space-y-4">
                {users.map(user => (
                  <div key={user.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                      <Select
                        value={user.role}
                        onValueChange={role => updateUserRole(user.id, role as User["role"])}
                ...
                <div className="flex justify-center gap-2 mt-4">
                  ...
                  <span className="px-2 py-1 text-sm">Page {userPage} of {userTotalPages}</span>
        ...
        {/* Article Management */}
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" /> Article Management</CardTitle></CardHeader>
          <CardContent>
            <div className="flex items-center gap-2 mb-4">
              <Input
              ...
              <div className="space-y-4">
                {articles.map(article => (
                  <div key={article.id} className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <div className="flex items-center space-x-2">
                      <Select value={article.status} onValueChange={status => updateArticleStatus(article.id, status as Article["status"])}>
                        <SelectTrigger className="w-32"><SelectValue /></SelectTrigger>
                        <SelectContent>
